...
from ._anvil_designer import AppStartTemplate
...
class Form1(Form1Template):
  ...
  def _request_ai_opening_move(self):
    ...
    # Only do this if it's actually AI's turn
    ...
    try:
      # Use keyword args to avoid positional-arg mismatch
  ...
  def drop_piece(self, col):
    ...
    # optimistic placement for HUMAN (current player)
  ...
  def restart_game(self, **e):
    ...
    # Resolve AI side (and resolve Random once per new game)
    self.ai_player = self._compute_ai_player()

    # Turn 0 always starts; if AI is Red (0), it's AI's turn immediately.
    self.player = 0

    self.render_board()
    self._update_status_ui()

    # Optional: announce the random resolution
    ...
    # If AI controls Red (0), request opening move immediately
...
class AppStart(AppStartTemplate):
  def __init__(self, **properties):
    self.init_components(**properties)
...
class LoginForm(LoginFormTemplate):
  ...
  def login_btn_click(self, **event_args):
    ...
    email = (self.email_tb.text or "").strip()
    password = self.password_tb.text or ""

    if not email or not password:
      return self._show_error("Enter both email and password.")
...
class MainForm(MainFormTemplate):
  def __init__(self, **properties):
    ...
    # Users rows typically have an 'email' field
    self.welcome_lbl.text = f"Welcome, {user['email']}!"
